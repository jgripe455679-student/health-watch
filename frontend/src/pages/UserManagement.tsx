...
const UserManagement: React.FC = () => {
  ...
  const handleSearch = useCallback(async () => {
    try {
      const response = await get(`users/search?username=${searchQuery}`);
      if (response.status === 200) {
        setUsers(response.data as User[]);
      }
    } catch (error) {
    ...
    <div className="px-8 py-1">
      <div className="card card-bordered border-gray-300 rounded-none shadow">
        <div className="card-body p-0">
          <h6 className="card-title bg-gray-200 text-sm text-primary p-1">
            User Management
          </h6>
          {successMessage && (
            <div
              ...
              <button
                className={
                  currentPage === pageNumber
                    ? "join-item rounded-none btn btn-xs btn-active"
                    : "join-item rounded-none btn btn-xs"
                }
                key={pageNumber}
  ...
  const UserCard = () => (
    <div className="px-8 py-1">
      <div className="card card-bordered border-gray-300 rounded-none shadow">
        <div className="card-body p-0">
          <h6 className="card-title bg-gray-200 text-sm text-primary p-1">
            {currentView === "editUser" && isEditing ? "Edit User" : "New User"}
          </h6>
          <UserForm
  ...
  return (
    <div className="h-full w-full">
      <Navbar />
      {currentView === "userManagement" ? <UserManagementCard /> : <UserCard />}
    </div>
  );
};
